...
`dot_config/mise/private_config.local.toml.tmpl` / `run_onchange_after_install-mise-tools.sh.tmpl` で使用中（private GitHub repo のツール導入用。任意。public repo に内部参照＝対象 repo 名・バージョンを残さないためテンプレート化）:
- `{{ .private_tool_repo }}` — private tool の `owner/repo`（プレースホルダ `<owner>/<repo>`）。github バックエンドの tool spec に使用。`hasKey` ゲートで設定マシンのみ導入
- `{{ .private_tool_version }}` — private tool のバージョン。repo 名と同様 public repo に残さないため注入する。`private_tool_repo` を設定しているのにこれが無いと chezmoi がキー未定義エラーで apply を止める（設定漏れの fail-fast）
...
- `run_onchange_after_install-local-skills.sh.tmpl` — **自作 skill の第3チャネル**。`dot_agents/skills-local/*/` に置いたスキルを `~/.agents/skills/` へ `cp -R` でコピーする。先頭コメントに各スキルファイルの sha256sum を埋め込んでいるため、**自作スキルのファイルを編集すると次の apply でコピーが再実行**される。GitHub lock チャネル（`run_onchange_after_install-skills.sh.tmpl`）は lock 外のフォルダを削除しないため、両チャネルのスキルは `~/.agents/skills/` 内で別フォルダとして共存できる。自作スキルを追加・編集したら、対応する `SKILL.md` / サポートファイルを `dot_agents/skills-local/<name>/` に置き、先頭コメントの hash 行にも `{{ include "dot_agents/skills-local/<name>/<file>" | sha256sum }}` を**ファイルごとに**追記してコミットすること（hash は明示列挙で、glob 展開しない）。**削除するとき**は source から `git rm` するだけでは足りず、① hash 行から該当エントリを除去 ② スクリプト内の `REMOVED_SKILLS` 配列に skill 名を追記（既存マシンの `~/.agents/skills/<name>/` を除去。lock チャネルの skill 名は書かないこと） ③ `.chezmoiremove` に target パス `.agents/skills-local/<name>` を追記（chezmoi は source から消えた entry を home から自動削除しないため、残すと `$SRC` 走査で毎回再インストールされる）——の3点を併せて行う。② ③ は全マシンが apply 済みになったらエントリを消してよい。
- `run_onchange_after_install-mise-tools.sh.tmpl` — 同じ仕組みで、ハッシュ行を **3 本**（`config.toml.tmpl` / `private_config.local.toml.tmpl` / `private_mise.lock`）埋め込んでおり、**いずれかが変わると次の apply で `mise install` が走る**。加えて `{{ .private_tool_version | sha256sum }}` の行があり、`include` では拾えない chezmoi.toml 側のバージョン変更でも再実行される（値そのものはレンダリング後にも残さない）。`mise.lock` も chezmoi 管理下なので、`after_` により install より先に配置される。`--locked` は付けない（private tool は `mise.local.lock` を持たず失敗するため）。`mise lock -g` もここでは実行しない（apply のたびに lock を書き換えて drift を作るため。lock 更新は人間の明示操作）。`after_` プレフィックスで `run_onchange_install-packages.sh.tmpl`（brew が mise 本体を入れる）の**後**に実行し、mise バイナリの存在を保証する。aqua バックエンドへ移行した CLI ツール群はここでインストールされる。mise 未検出時は apply を止めずスキップ。private GitHub repo のツールを github バックエンドで入れるため、`mise install` の直前に op-vault (`mise exec "github:sunakan/op-vault"`) で 1Password から `GITHUB_TOKEN` を解決・export する。この処理は `{{ if hasKey . "private_tool_repo" }}` ゲートで囲まれ、変数を設定したマシンでだけ有効（未設定マシンでは private tool 行が config にも出ず、apply は通常成功）。トークンを解決できなければ apply を**中断**する（他ツールも止まるが silent skip はしない）。
- `run_onchange_after_install-claude-plugins.sh.tmpl` — Claude Code プラグイン導入の**唯一の source of truth**。スクリプト先頭の inline manifest（`MARKETPLACES` / `PLUGINS` の bash 配列）に marketplace の `name repo` と plugin の `<plugin>@<marketplace>` を列挙し、`claude plugin marketplace add`（既存なら `update`）→ `claude plugin install` を冪等に流す。`claude plugin install` が cache 本体を実体化する（`known_marketplaces.json` に記録されるだけでは `~/.claude/plugins/cache/` に clone されず、`claude plugin list` 上 enabled でも skill/hook がロードされない）。run_onchange なので manifest を編集するとスクリプト内容が変わり次の apply で再実行される（別途ハッシュ埋め込みは不要）。`after_` プレフィックスで mise が `claude` 本体を入れた**後**に実行。`claude` 未検出時は apply を止めずスキップ。**プラグインの増減は manifest 配列を編集するだけ**（JSON への手動反映・`chezmoi re-add` は不要）。Claude Code が実行時所有する `~/.claude/plugins/{known_marketplaces,installed_plugins}.json` は `.chezmoiignore` 済みで**追跡しない**（タイムスタンプ等のドリフトを止めるため。以前は静的 JSON 管理で `chezmoi diff` が出続けていた）。
- `dot_claude/modify_settings.json.tmpl` — `~/.claude/settings.json` は「手で管理したい設定（env/permissions/model/hooks/deny 等）」と「Claude Code が実行時に書き換えるキー（`enabledPlugins`/`extraKnownMarketplaces`/`feedbackSurveyState`・キー順）」が混在するハイブリッドファイル。静的管理すると後者が毎回 drift するため chezmoi `modify_` スクリプト化した。現在のファイルを stdin で受け取り stdout が新ファイルになる仕組みで、node の spread マージ（`{ ...cur, ...managed }`）により管理キーだけ値を強制し、Claude 所有キーは位置ごと保持する（既存マシンでは出力がバイト一致 → diff ゼロ）。新規マシンでは `enabledPlugins` を seed。**安全策**: 出力がそのままファイルになるため、`node` 不在・JSON パース失敗時は `cat`（現状維持）でフォールバックし settings.json を破壊しない。herdr フックパスは `{{ .chezmoi.homeDir }}` を env 経由で渡してクォートのネストを回避している。

依存パッケージ（`powerlevel10k` 等）は必ず `Brewfile.tmpl` に追加すること。`dot_zshrc` から参照するだけで Brewfile に無いと、新規マシンで未インストールになり壊れる。`Brewfile.tmpl` は Go template として評価されるので、マシンによって入れ分けたい cask（container runtime 等）は `{{ if hasKey . "install_xxx" }}` ゲートで囲み、chezmoi.toml の `[data]` にキーを設定したマシンでだけ install されるようにすること（orbstack / docker-desktop が例）。主要な single-binary CLI ツール群は Homebrew から aqua バックエンドへ移行済み。新規 CLI ツールは原則 Brewfile ではなく `dot_config/mise/config.toml.tmpl` の `[tools]` に `"aqua:<owner>/<repo>" = "<version>"` 形式で追加すること。`brew info --json=v2 <formula>` で上流 owner/repo を確認し、`mise ls-remote aqua:<id>` でバージョン実在を確認してから追記する。GNU/system 系・bootstrap ツール（chezmoi/mise）・等価性が曖昧なもの（tmux/aws-vault/coreutils）は引き続き Homebrew が担う。**`gcloud` は single-binary ではないが例外的に aqua 管理**（`aqua:twistedpair/google-cloud-sdk`。実体は Google 公式 tarball で、gcloud/gsutil/bq/docker-credential-gcloud を同梱）。ただし ① `gcloud components install/update` は mise の install dir に書くためバージョン変更で消え、pin とも競合する ② バージョン上限が非公式ミラー repo のタグ更新に縛られる、の2点に注意すること（詳細は `dot_config/mise/config.toml.tmpl` の NOTE）。**private repo のツールは例外**として、aqua レジストリに載らないため github バックエンド（`"github:<owner>/<repo>" = "<version>"`）で入れ、認証用 `GITHUB_TOKEN` を op-vault 経由で解決する。対象 repo 名・op 参照はテンプレート変数（`private_tool_repo` 等）で注入し public repo に残さず、`hasKey` ゲートで設定マシンのみ opt-in にする（上記の mise-tools スクリプトとテンプレート変数を参照）。
...
mise のツールを追加・バージョン変更したら、lockfile (`dot_config/mise/private_mise.lock`) を source へ同期してコミットすること。**手順・検証コマンド・落とし穴は `mise-lock-update` skill (`.agents/skills/mise-lock-update/SKILL.md`) が正本**なので、作業時はそれを参照する（Codex は `$mise-lock-update`、Claude Code は `/mise-lock-update`）。ここに残す不変ルールは1点: **private repo のツールは `dot_config/mise/config.toml.tmpl` に書かないこと** — lockfile `mise.lock` に repo 名とリリースアセット URL が載り、public repo に出せなくなる。private tool は `dot_config/mise/private_config.local.toml.tmpl`（→ `~/.config/mise/config.local.toml`、lock は追跡しない `mise.local.lock` に分離される）に書く。`conf.d/*.toml` は lockfile が `mise.lock` に合流するので隔離手段にならない。
...
## このリポジトリ固有の skill (`.agents/skills/`)

本リポジトリ専用の skill は source root の `.agents/skills/<name>/SKILL.md` に置く（現在: `mise-lock-update`）。`.claude/skills` は `../.agents/skills` への相対 symlink とし、Codex と Claude Code が同じ正本を読む。chezmoi は `.` 始まりの source エントリを無視するので **chezmoi 管理外・git 追跡のみ**で、置くだけで有効。user-level の 2 チャネルとは別物で、**ハッシュ行・`REMOVED_SKILLS`・`.chezmoiremove`・`cp` 同期はいずれも不要**。全プロジェクトで使うなら `dot_agents/skills-local/` 側へ。
