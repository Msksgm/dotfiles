...
> このファイルは `@rules/testing.md` を Kotlin および Android/KMP 固有の内容で拡張する。

## テストフレームワーク

- **kotlin.test** — マルチプラットフォーム（KMP）向け: `@Test`, `assertEquals`, `assertTrue`
- **JUnit 4/5** — Android 固有テスト向け
- **Turbine** — Flow / StateFlow のテスト向け
- **kotlinx-coroutines-test** — コルーチンのテスト向け（`runTest`, `TestDispatcher`）
...
## モックよりフェイク（Fakes over Mocks）

モックフレームワークより手書きのフェイクを優先すること:
...
`runTest` を使うこと — 仮想時間を自動的に進め、`TestScope` を提供する。
...
## Room / SQLDelight のテスト

- Room: インメモリテストには `Room.inMemoryDatabaseBuilder()` を使う
- SQLDelight: JVM テストには `JdbcSqliteDriver(JdbcSqliteDriver.IN_MEMORY)` を使う
...
最低カバレッジ基準: 各機能の ViewModel + UseCase をカバーすること。
